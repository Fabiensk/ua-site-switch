...
function configAll(evt) {
    var panels = require('panel');
    var allConfigDlg = panels.Panel({
	width: 600,
	height: 480,
	contentURL: require("self").data.url('all.html'),
	contentScriptFile: [data.url('all.js'), data.url('jquery-1.7.2.min.js')],
	onMessage: function(site) {
		// console.log("Removing:", site, simpleStorage.storage.mapping[site]);
		delete simpleStorage.storage.mapping[site];
		// console.log(simpleStorage.storage.mapping[site]);
	},
	onShow: function() {	
		this.postMessage(simpleStorage.storage.mapping);
	}
...
exports.main = function() {
    ...
    // icon in the module bar
    new Widget({
        id: "UA-site-switch-cfg",
        label: "UA-site-switch-cfg",
        content: "Cfg",
        width: 50,
        // Add a function to trigger when the Widget is clicked.
        onClick: configAll
